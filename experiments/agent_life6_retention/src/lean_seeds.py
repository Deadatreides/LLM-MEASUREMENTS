"""lean_seeds.py — копия (не импорт) `agent_life5_slot_map/src/
lean_seeds.py`, без изменений. Бедный старт, Imp-пары найдены поверх
ПЕРЕИСПОЛЬЗОВАННОЙ (скопированной) сетки/панели LIFE-3/4/5 -- те же
PASS-паттерны, тот же грид. `M.random_genotype` не используется (LIFE-2
установила: он не производит валидных ASSEMBLE на реестре HETEROSTEP) --
все генотипы строятся явно из `heterostep_seeds.route`.

LIFE-6: импорт `ML` указывает на `metrics_lib` (LIFE-6's имя для
Imp-core модуля, см. PROTOCOL.md/README.md) -- сама функция `imp_set` и
её сигнатура (с `registry`-параметром) не изменились с LIFE-4.
"""
