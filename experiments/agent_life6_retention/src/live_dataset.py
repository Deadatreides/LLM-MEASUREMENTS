...
живой сетки (LIFE-6 PROTOCOL.md §0: grid=reuse -- скопирована byte-identical
из `agent_life5_slot_map/metrics/live_grid/` (сама переиспользована из
`agent_life4_block_fix` <- `agent_life3_block_live`), md5 проверен до
первого evolve, см. BLOCKERS.md). ТОТ ЖЕ сплит/seed, что LIFE-3/4/5
(GEN_SEED=150002/SPLIT_SEED=20260821) -- намеренно: тот же грид, не новый.
Копия (не импорт — изоляция пакетов) `agent_life5_slot_map/src/
...
tasks14 = _load_module("life6_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life6_seams14", EXP14 / "seams.py")
mr11 = _load_module("life6_model_registry11", EXP11_CONFIGS / "model_registry.py")

# -- LIFE-6 PROTOCOL.md: grid=reuse -- ТЕ ЖЕ значения, что agent_life3/4/5,
