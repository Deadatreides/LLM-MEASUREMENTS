"""live_dataset.py — bootstrap пакета + LiveDataset поверх СОБСТВЕННОЙ
живой сетки (строится этим пакетом с нуля, PROTOCOL.md §3.1) -- НЕ читает
`agent_a5_live_m`'s сетку как источник fitness (запрещено §1). Копия (не
импорт — изоляция пакетов) `agent_a5_live_m/src/live_dataset.py`'s узора,
адаптированная под новый сплит/seed'ы.
"""
...
tasks14 = _load_module("life3_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life3_seams14", EXP14 / "seams.py")
mr11 = _load_module("life3_model_registry11", EXP11_CONFIGS / "model_registry.py")

# -- PROTOCOL.md §3.1: НОВЫЙ сплит, не пересекается с A5/LIFE-1/LIFE-2 --
GEN_SEED = 150002
SPLIT_SEED = 20260821
N_TASKS = 200
LIVE_SEED_BASE = 6000

TEMPERATURE = HSTEP.TEMPERATURE            # 0.5, переиспользовано -- см. §3.1 обоснование
...
def stable_hash(task_id: str, kind: str, model_id: str) -> int:
    """PROTOCOL.md §3.1: детерминировано между прогонами -- НЕ встроенный
    Python hash() (рандомизирован по процессу для строк)."""
...
class LiveDataset:
    """Аналог `agent_a5_live_m.src.live_dataset.LiveDataset`, но со своим
    сплитом/сеткой (PROTOCOL.md §3.1) -- этот пакет строит живую сетку
    сам, не читает A5's."""
