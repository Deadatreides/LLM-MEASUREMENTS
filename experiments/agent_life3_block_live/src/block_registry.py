...
сам акт регистрации (переиспользует `arch2.heterostep.Registry.
register_composite` БЕЗ ИЗМЕНЕНИЙ -- блок оборачивается в самостоятельный
генотип, регистрируется под СОБСТВЕННЫМ complex_id).

Черновика LIFE-3 с готовыми R1-R4 в репозитории не найдено -- правила
определены здесь из численных параметров задания, см. PROTOCOL.md §4.2
для полного обоснования (по образцу `arch2.heredity.registration_gate`/
`evolve._try_register_molecule`, применённого на уровне слота).
"""
...
def block_signature(slot_node: dict) -> tuple:
    """PROTOCOL.md §4.2: (op, sorted(models)) -- та же `signature_s`, что
    LIFE-1/2, здесь используется как РЕГИСТРИРУЕМЫЙ ключ, не только для Imp."""
    op = slot_node.get("op")
    models = tuple(sorted(ML.slot_models(slot_node)))
    return (op, models)
