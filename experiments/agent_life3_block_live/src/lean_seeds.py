"""lean_seeds.py — PROTOCOL.md §5: бедный старт, Imp-пары найдены ЗАНОВО
на этой (новой) сетке -- не скопированы с LIFE-2's старой панели (панель
другая, PASS-паттерны другие живых ответов). `M.random_genotype` не
используется (LIFE-2 установила: он не производит валидных ASSEMBLE на
реестре HETEROSTEP) -- все генотипы строятся явно из `heterostep_seeds.
route`.
"""
...
def greedy_order_deterministic(ds, panel: list, kind: str) -> list:
    """Копия `agent_life2_complementary/src/lean_seeds.py::
    greedy_order_deterministic` -- алфавитный тай-брейк, не хэш-порядок-
    зависимый `heterostep_seeds._greedy_order`."""
    covered: set = set()
    chosen: list = []
    rest = set(ds.models)
    while rest:
...
def find_robust_pairs(ds, panel: list, best_single: dict) -> dict:
    ...
    Imp(slot)=1 на ЭТОЙ панели. Найдено заново (не скопировано с LIFE-2),
    т.к. панель/сетка другие. Одномодельный слот структурно не может дать
    Imp=1 -- минимум 2 модели, проверено конструкцией (PROTOCOL.md §4.2)."""
...
def build_seeds_pop20_lean(ds, panel: list, best_single: dict) -> dict:
    ...
    разными именами (найдено смоук-тестом одной клетки)."""
...
def assert_lean_population(seeds: dict, ds, panel: list, best_single: dict) -> None:
    """PROTOCOL.md §5: два обязательных assert'а до generation 0."""
