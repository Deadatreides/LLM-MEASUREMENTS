"""lean_seeds.py — копия (не импорт) `agent_life6_retention/src/
lean_seeds.py`, без изменений. Бедный старт, Imp-пары найдены поверх
ПЕРЕИСПОЛЬЗОВАННОЙ (скопированной) сетки/панели LIFE-3..6 -- те же
...
Импорт `ML` указывает на `metrics_lib` (единое имя для Imp-core модуля
во всей линии LIFE-6/7) -- сама функция `imp_set` и её сигнатура (с
`registry`-параметром) не изменились с LIFE-4.
"""
