...
Копия (не импорт — изоляция пакетов agent_aN) `agent_a4_m_plateau/src/
recombination.py`. Оператор сам по себе не меняется: слот выбирается с
уклоном P(LOOKUP)=P(COMPUTE)=2*P(READ)=2*P(FORMAT), вероятность применения
— внешний параметр (`p_cross=0.60` фиксирован, PROTOCOL.md §6), не
константа здесь.
"""
...
# PROTOCOL.md §6: P(LOOKUP)=P(COMPUTE)=2*P(READ)=2*P(FORMAT), веса на слоты
...
def swap_assemble_slot(parent: dict, rng: random.Random, ctx: dict, registry, gen: int) -> Tuple[Optional[dict], dict]:
    ...
    (взвешивание через `ctx["donor_score_fn"]`, если задан). При неудаче (нет
    валидных ASSEMBLE-доноров, результат невалиден/дубликат) -- до
    MAX_RETRIES попыток, затем откат на немодифицированный
    `arch2.mutate.mutate(...)`.
    """
    ...
    for _ in range(MAX_RETRIES):
        ...
        child = G.genotype(new_root, gen=gen, parent_ids=[parent["complex_id"]],
                           origin="mutate:A5_swap_assemble_slot")
