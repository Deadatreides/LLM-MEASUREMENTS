"""live_grid_builder.py — PROTOCOL.md §4: единственная тяжёлая фаза
generate(). Модель-мажорный обход (тот же порядок, что
experiment14/harness.py::run_train_grid доказанно прогнал быстро: 2400
ячеек за 517с), но: (а) НОВЫЙ сплит/seed (live_dataset.py), (б)
детерминированный per-ячейка seed (PROTOCOL.md §3), (в) резюмируемо -- уже
записанные на диске ячейки не перевызываются, (г) каждый вызов -- отдельная
строка live_call_log.jsonl (PROTOCOL.md §9).
"""
...
def build_live_grid(task_ids: list, tasks: dict, split: dict) -> dict:
    """-> {(task_id, kind, model): row}. Резюмируемо: ячейки, уже
    присутствующие на диске в LIVE_TRAIN_GRID/LIVE_TEST_GRID, не
    перевызываются (PROTOCOL.md §1: повторный generate() для уже успешно
    записанной ячейки запрещён)."""
    cells = _load_existing_cells()
    train_ids = set(split["train"])
    n_before = len(cells)
    print(f"[a5-grid] на диске уже {n_before} ячеек (резюме)", flush=True)

    for model_id in MODEL_IDS:
        n_todo = sum(1 for t in task_ids for k in STEP_KINDS if (t, k, model_id) not in cells)
        if n_todo == 0:
            print(f"[a5-grid] {model_id} уже полностью на диске, пропуск загрузки", flush=True)
            continue
        llm, load_t = LD.mr11.load_model(model_id)
        print(f"[a5-grid] {model_id} загружена за {load_t:.1f}с — {n_todo} ячеек в очереди", flush=True)
        t0 = time.time()
        n_done_this_model = 0
        for task_id in task_ids:
            ...
            for step_idx, kind in enumerate(STEP_KINDS):
                ...
                row = {
                    "experiment_id": "a5_live_m", "phase": "grid", "task_id": task_id,
                ...
                if n_done_this_model % 200 == 0:
                    print(f"[a5-grid]   {model_id}: {n_done_this_model}/{n_todo}", flush=True)
        del llm
        print(f"[a5-grid] {model_id} готова за {time.time() - t0:.0f}с "
...
if __name__ == "__main__":
    ...
    floor = 6 * 4 * 120
    print(f"[a5-grid] ИТОГО: {n_rows}/{target} ячеек за {time.time() - t_start:.0f}с "
          f"(приёмочный минимум {floor})")
    if n_rows < floor:
        print("[a5-grid] НИЖЕ ПРИЁМОЧНОГО МИНИМУМА -- писать в BLOCKERS.md, не маскировать", flush=True)
        sys.exit(1)
    print("[a5-grid] OK", flush=True)
