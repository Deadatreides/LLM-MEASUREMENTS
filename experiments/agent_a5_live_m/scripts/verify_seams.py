"""verify_seams.py — PROTOCOL.md §1/§9, шаг 0-1: швы + РОВНО один смоук
generate(). Обязателен ДО live_grid_builder.py. Провал -> BLOCKERS.md, стоп,
без отката на offline (PROTOCOL.md §1/§9).
"""
...
def smoke_generate() -> dict:
    """Ровно один живой вызов generate(), ровно одна строка в live_call_log.jsonl."""
    ds = LD.default_dataset()
    task_id = ds.split["train"][0]
    task = ds.tasks[task_id]
    kind = LD.HSTEP.STEP_KINDS[0]          # READ, depends_on=[] -- не требует upstream
    step_idx = 0
    step = task["steps"][step_idx]
    model_id = LD.HSTEP.MODEL_IDS[0]       # llama-3.2-1b-instruct-q4_0 -- самая быстрая загрузка
...
if __name__ == "__main__":
    ...
    ok = verify_seams()
    if not ok:
        print("[verify] ШВЫ НЕ ПРОШЛИ — СТОП (PROTOCOL.md §1: без offline-отката)")
        sys.exit(1)
    try:
        smoke_generate()
