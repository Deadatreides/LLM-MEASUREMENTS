...
Отличие от `agent_a2_assemble_budget/src/recombination.py::
enhanced_m5_cross_slot`: там слот выбирался равномерно (`rng.randrange`), и
вероятность применения была зашита константой 0.40. Здесь (PROTOCOL.md §4):
слот выбирается с уклоном P(LOOKUP)=P(COMPUTE)=2*P(READ)=2*P(FORMAT), а
вероятность применения — внешний параметр `p_cross`, задаваемый вызывающим
кодом (`src/orchestrator.py::custom_reproduce`), не константа здесь.
"""
...
def swap_assemble_slot(parent: dict, rng: random.Random, ctx: dict, registry, gen: int) -> Tuple[Optional[dict], dict]:
    ...
    `arch2.mutate.mutate(...)` (PROTOCOL.md §4: "остальные акты -- baseline").
    """
    ...
    for _ in range(MAX_RETRIES):
        ...
        child = G.genotype(new_root, gen=gen, parent_ids=[parent["complex_id"]],
                           origin="mutate:A3_swap_assemble_slot")
