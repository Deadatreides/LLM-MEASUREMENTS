...
живой сетки (LIFE-8 PROTOCOL.md P1: grid=reuse -- скопирована byte-identical
из `agent_life7_selection/metrics/live_grid/` (сама переиспользована из
`agent_life6_retention` <- `agent_life5_slot_map` <- `agent_life4_block_fix`
<- `agent_life3_block_live`), md5 проверен до первого evolve, см.
BLOCKERS.md). ТОТ ЖЕ сплит/seed, что LIFE-3..7 (GEN_SEED=150002/
SPLIT_SEED=20260821) -- намеренно: тот же грид, не новый. Копия (не
импорт — изоляция пакетов) `agent_life7_selection/src/live_dataset.py`'s
узора, путь к сетке указывает на СОБСТВЕННУЮ (скопированную) директорию
этого пакета.

LIFE-8 расширение (единственное изменение против LIFE-7's версии):
`LiveDataset.__init__` принимает опциональный `extra_paths` -- Phase 3
(`model_expansion.py`) конструирует ОТДЕЛЬНЫЙ `LiveDataset(extra_paths=
(new_models_grid_path,))` ПОСЛЕ патча `HSTEP.MODEL_IDS` (см. PROTOCOL.md
коррекция #5) -- `self.models = HSTEP.MODEL_IDS` на строке ниже тогда
естественно подхватывает расширенный список, без отдельного параметра.
По умолчанию `extra_paths=()` -- Phase 1/2 получают БУКВАЛЬНО то же
поведение, что LIFE-7's версия.
"""
...
tasks14 = _load_module("life8_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life8_seams14", EXP14 / "seams.py")
mr11 = _load_module("life8_model_registry11", EXP11_CONFIGS / "model_registry.py")

# -- LIFE-8 PROTOCOL.md P1: grid=reuse -- ТЕ ЖЕ значения, что agent_life3..7,
...
NEW_MODELS_GRID = LIVE_GRID_DIR / "new_models_grid.json"   # Phase 3 only, may not exist
...
class LiveDataset:
    ...
    же сплит/сетка (скопирована, не перестроена -- grid=reuse). `extra_
    paths` -- LIFE-8's единственное добавление (Phase 3, см. модульный
    docstring)."""
...
def default_dataset(*, fresh: bool = False) -> LiveDataset:
    """Phase 1/2's ORIGINAL 6-model dataset only -- cached singleton, as
    in every prior package. Phase 3 constructs its OWN `LiveDataset(
    extra_paths=(NEW_MODELS_GRID,))` directly, never through this cache
    (a different alphabet needs a genuinely different instance, not a
    mutated shared one)."""
    global _DATASET
