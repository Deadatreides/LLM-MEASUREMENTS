...
`agent_life3_block_live/src/slot_hgt.py`, БЕЗ ИЗМЕНЕНИЙ (LIFE-4 PROTOCOL.md
§0: ровно два изменения -- unfold cx в metrics_lib + окно вымирания в
orchestrator -- этот файл ни то, ни другое, p=0.35, комплементарность как
единственный критерий донора, `donor_score_fn` нигде в `ctx`, откат на
`arch2.mutate.mutate`).
"""
