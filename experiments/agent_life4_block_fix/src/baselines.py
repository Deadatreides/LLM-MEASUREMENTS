"""baselines.py — копия (не импорт) `agent_life3_block_live/src/
baselines.py`, БЕЗ ИЗМЕНЕНИЙ. B0-B3 на ПЕРЕИСПОЛЬЗОВАННОЙ живой сетке
(LIFE-4 PROTOCOL.md: grid=reuse), вторичные метрики (не заголовок
вердикта). B1-B3 -- чистая конструкция генотипа из уже собранной сетки
(0 новых вызовов); B0 -- читает `live_grid/whole_grid.json`, тоже уже
скопированный из LIFE-3 -- 0 новых вызовов при reuse (данные решают
