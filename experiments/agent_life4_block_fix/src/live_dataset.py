...
живой сетки (LIFE-4 PROTOCOL.md: grid=reuse -- скопирована byte-identical
из `agent_life3_block_live/metrics/live_grid/`, md5 проверен до первого
generate()-adjacent кода, см. BLOCKERS.md). ТОТ ЖЕ сплит/seed, что LIFE-3
(GEN_SEED=150002/SPLIT_SEED=20260821) -- намеренно: это тот же грид, не
новый. Копия (не импорт — изоляция пакетов) `agent_life3_block_live/src/
live_dataset.py`'s узора, путь к сетке указывает на СОБСТВЕННУЮ (скопированную)
директорию этого пакета, не на `agent_life3_block_live/`.
"""
...
tasks14 = _load_module("life4_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life4_seams14", EXP14 / "seams.py")
mr11 = _load_module("life4_model_registry11", EXP11_CONFIGS / "model_registry.py")

# -- LIFE-4 PROTOCOL.md: grid=reuse -- ТЕ ЖЕ значения, что agent_life3_block_live,
