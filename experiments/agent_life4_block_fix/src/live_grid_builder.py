"""live_grid_builder.py — копия (не импорт) `agent_life3_block_live/src/
live_grid_builder.py`, БЕЗ ИЗМЕНЕНИЙ. LIFE-4 PROTOCOL.md: grid=reuse
(скопирован готовым из agent_life3_block_live/metrics/live_grid/,
целостность проверена md5 -- см. BLOCKERS.md) -- этот модуль НЕ
запускается в штатном прогоне, держится только как UNIT4_BLOCKED-
fallback, если проверка целостности копии когда-либо провалится (полная
пересборка, модель-мажорный обход, резюмируемо, каждый вызов -- строка
`live_call_log.jsonl`).
"""
