"""block_insert.py — копия (не импорт) `agent_life3_block_live/src/
block_insert.py`, БЕЗ ИЗМЕНЕНИЙ (LIFE-4 PROTOCOL.md §0: этот файл не
вызывает `slot_models`/`imp_set` напрямую, только через `imp_fn`,
которая патчится в `orchestrator.py` -- см. Change 1). Вставляет CALL на
ЗАРЕГИСТРИРОВАННЫЙ блок (композит) в слот, предпочитая комплементарные
слоты (`Imp(s,parent)=0`), но не требуя этого жёстко (в отличие от
`transfer_slot` -- "предпочтительно", см. PROTOCOL).
"""
