...
def _token_entropies(lp):
    """Truncated top-K entropy per generated token, H_t = -sum p log2 p
    over just the K candidates llama.cpp returned (the missing probability
    mass outside top-K is not accounted for -- tracked explicitly as
    `captured_mass` so the truncation is visible, not hidden)."""
    entropies = []
    captured_mass = []
    chosen_logprob = []
    top_lp_list = lp.get("top_logprobs") or []
    tok_lp_list = lp.get("token_logprobs") or []
    for i, top in enumerate(top_lp_list):
        probs = [math.exp(float(v)) for v in top.values()]
        mass = sum(probs)
        h = -sum(p * math.log2(p) for p in probs if p > 0)
        entropies.append(round(h, 4))
        captured_mass.append(round(mass, 4))
        chosen_logprob.append(round(float(tok_lp_list[i]), 4) if i < len(tok_lp_list) else None)
    return {
        "tokens": lp.get("tokens"),
        "entropy_per_token_bits": entropies,
        "captured_mass": captured_mass,
        "chosen_token_logprob": chosen_logprob,
        "top_k": len(top_lp_list[0]) if top_lp_list else None,
    }


def load_model(model_id, logits_all=False):
    entry = get_model_entry(model_id)
    path = os.path.join(MODELS_BASE, entry["path"])
    t0 = time.time()
    llm = Llama(model_path=path, n_gpu_layers=-1, n_ctx=N_CTX, verbose=False, logits_all=logits_all)
    load_time_sec = time.time() - t0
    return llm, entry, load_time_sec


def generate(llm, model_id, model_entry, user_content, temperature=0.0, top_p=1.0, top_k=40, seed=1, max_tokens=700, logprobs=None):
    ...
    try:
        ...
        entropy_trace = _token_entropies(lp) if lp else None
        return {
