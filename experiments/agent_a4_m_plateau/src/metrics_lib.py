...
Копия (не импорт) `agent_a3_mstar/src/metrics_lib.py` — формула U/P/Coverage/
P(A,B) НЕ изменена (третий раз копируется без изменения: A2 -> A3 -> A4,
изоляция пакетов, PROTOCOL.md §1). Единственное добавление относительно A3:
`archive_verdicts` теперь возвращает `best_nodes` (число узлов лучшего по
(r,-c) генотипа) -- нужно PROTOCOL.md §5 A4, отсутствовало в A3.
"""
...
# -- канонические базовые линии (PROTOCOL.md §2/§5, фиксированы, не пересчитываются по срезу) --
...
# -- archive-вердикты (PROTOCOL.md §5, формула идентична A3) -----------------------------------
...
def archive_verdicts(archive_genotypes: List[dict], ds, reg, be, train_ids: list) -> dict:
    """PROTOCOL.md §5: переоценка ВСЕГО архива (без эталонов, исключены ДО
    вызова этой функции) на полной сетке, сравнение с ФИКСИРОВАННЫМИ
    каноническими B1/B2/B3 (не пересчитанными по срезу)."""
    evaluated = []
    ...
    if evaluated:
        ...
        best_nodes = G.n_nodes(best_g["root"])
    else:
        best_r, best_c, best_nodes = 0.0, float("inf"), 0

    return {
        "v1_archive": v1, "v2_archive": v2, "v3_archive": v3,
        "best_r_archive": best_r, "best_c_archive": best_c if best_c != float("inf") else -1.0,
        "best_nodes": best_nodes, "n_evaluated": len(evaluated),
    }
