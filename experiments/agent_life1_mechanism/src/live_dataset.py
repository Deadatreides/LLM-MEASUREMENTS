...
живой сетки `agent_a5_live_m/` (только чтение, PROTOCOL.md §7). Копия (не
импорт — изоляция пакетов agent_aN) `agent_a5_live_m/src/live_dataset.py`,
единственное отличие — пути к grid-файлам указывают в `agent_a5_live_m/
metrics/live_grid/`, не в собственную папку (этот пакет не строит сетку
заново, PROTOCOL.md §0/§7).
"""
...
import heterostep_seeds as HSEED  # noqa: E402  arch2/heterostep_seeds.py -- .route/._slot/._fallback
                                   ...
                                   # PROTOCOL.md §5) -- see deterministic_route.py
...
tasks14 = _load_module("life1_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life1_seams14", EXP14 / "seams.py")

# -- те же seed'ы, что A5 (PROTOCOL.md §0: тот же split, тот же архивный прогон) --
GEN_SEED = 150001
SPLIT_SEED = 20260820
N_TASKS = 200
...
# -- живая сетка A5, ТОЛЬКО ЧТЕНИЕ (PROTOCOL.md §0/§7) --
...
class LiveDataset:
    """Идентична `agent_a5_live_m.src.live_dataset.LiveDataset` по форме
    (`.tasks`/`.split`/`.cells`/`.models`/`.kinds`/`.initial_state`/
    `.coverage`), но указывает на A5's grid-файлы -- этот пакет их не
    строит и не изменяет."""
