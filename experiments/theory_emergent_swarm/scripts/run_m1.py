"""run_m1.py — M1: four arms isolating decomposition / swarm / МДП.
...
import task_generator_m1 as M1     # noqa: E402
...
def ask(llm, model_id, key, prompt, max_tokens):
    ...
    _cache[key] = txt
    _calls += 1
    call_log.log_filter_call(model=model_id, task_id=str(key[0]), family=f"M1-{key[1]}",
...
def main() -> None:
    ...
    for mi, model_id in enumerate(models):
        ...
        for tid in ids:
            task = tb[tid]
            whole.setdefault(tid, {})[model_id] = OR.extract_number(
    ...
    def summed(task, emp_ids):
        valid = [i for i in emp_ids if i in task["id_to_salary"]]     # ВДП: drop hallucinations
        return round(sum(task["id_to_salary"][i] for i in valid), 2) if valid else None

    def ok(task, val):
        return val is not None and OR.within_tolerance(val, task["final_oracle"])
    ...
    (METRICS / "m1_result.json").write_text(json.dumps({
