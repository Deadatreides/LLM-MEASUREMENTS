"""lean_seeds.py — копия (не импорт) `agent_life4_block_fix/src/
lean_seeds.py`, только для Phase B.2 (optional evolve). Бедный старт,
Imp-пары найдены поверх ПЕРЕИСПОЛЬЗОВАННОЙ (скопированной) сетки/панели
LIFE-3/4 -- те же PASS-паттерны, тот же грид. `M.random_genotype` не
...
LIFE-5 адаптация (единственное изменение против LIFE-4's версии): импорт
`ML` теперь указывает на `unfold_metrics` (LIFE-5's имя для Imp-core
модуля, см. PROTOCOL.md §5/Phase B), не `metrics_lib` -- сама функция
`imp_set` и её сигнатура (с `registry`-параметром) не изменились.
"""
