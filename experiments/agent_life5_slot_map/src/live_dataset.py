...
живой сетки (LIFE-5 PROTOCOL.md §0/Phase B.2: grid=reuse -- скопирована
byte-identical из `agent_life4_block_fix/metrics/live_grid/` (которая сама
переиспользована из `agent_life3_block_live`), md5 проверен до первого
generate()-adjacent кода, см. BLOCKERS.md). ТОТ ЖЕ сплит/seed, что LIFE-3/4
(GEN_SEED=150002/SPLIT_SEED=20260821) -- намеренно: это тот же грид, не
новый. Используется ТОЛЬКО Phase B.2 (optional evolve) -- Phase B (карта)
не открывает эту сетку вообще, читает уже готовые archive.json/summary.json.
Копия (не импорт — изоляция пакетов) `agent_life4_block_fix/src/
...
tasks14 = _load_module("life5_tasks14", EXP14 / "tasks" / "heterostep.py")
seams14 = _load_module("life5_seams14", EXP14 / "seams.py")
mr11 = _load_module("life5_model_registry11", EXP11_CONFIGS / "model_registry.py")

# -- LIFE-5 PROTOCOL.md Phase B.2: grid=reuse -- ТЕ ЖЕ значения, что agent_life3/4,
