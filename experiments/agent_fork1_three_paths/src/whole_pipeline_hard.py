"""whole_pipeline_hard.py — FORK-1 Path B, K5: r_m*_B whole-task
measurement for every registry model on T_hard. PROTOCOL.md §2.4: whole
prompt asks ONLY for the final number (no intermediates elicited);
`cost = input+output`; `cost>B_hard -> FAIL` (not "almost"); same
principle as DELTA-0's own whole pipeline, reapplied not rederived.
"""
...
TEMPERATURE = 0.0
SEED_BASE_WHOLE_HARD = 90000
MAX_TOKENS_WHOLE_GEN = 30
...
def draw_seed(task_id: str, model_id: str) -> int:
    return SEED_BASE_WHOLE_HARD + stable_hash(task_id, model_id)
...
def measure_whole_for_model(model_id: str, tasks_by_id: dict, task_ids: list, B: int,
    ...
    print(f"[whole-hard] {model_id} загружена за {load_t:.1f}с", flush=True)
    results = {}
    for t_id in task_ids:
        ...
        if failed:
            ...
            v, over_budget = 0, True
        else:
            check = OR.check_aggregate(raw.get("raw_text", ""), task["final_oracle"])
...
def select_m_star(r_m_table: dict) -> str:
    ...
    smaller model_id (same rule DELTA-0 used)."""
